...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `SerdePathToError.swiftmodule/`
...
//      `import SerdePathToError` above would fail at compile time.
//
//   2. The static archive `libSerdePathToError.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "SerdePathToError" }` and
//      `flattenPackage = "io.github.kotlinmania.serdepathtoerror"` configuration in
...
//      via the `SerdePathToErrorLibrary` product.
//
...
final class SerdePathToErrorExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "SerdePathToError swift module imported cleanly")
    }
}
