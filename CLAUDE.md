# Claude Code Project Instructions — serde-path-to-error-kotlin

## Project Overview

This is **serde-path-to-error-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`serde_path_to_error`](https://crates.io/crates/serde_path_to_error).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/serde_path_to_error/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/serde_path_to_error>
